...
def evaluate_against_random(agent, num_games=100):
    ...
    print(f"PPO TicTacToe Results: {win_rate:.1f}% Wins, {draw_rate:.1f}% Draws, {loss_rate:.1f}% Losses")
    return win_rate

def main():
    print(f"Train PPO on TicTacToe ({SIZE}x{SIZE}, win_length={WIN_LENGTH})")
    print(f"Vectorized Environments: {NUM_ENVS} parallel games. Batch size: {BATCH_SIZE}")
    
    agent = PPOAgent(board_size=SIZE)
